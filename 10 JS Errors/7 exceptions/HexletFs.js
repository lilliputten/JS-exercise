...
export default class {
  ...
  copySync(src, dest) {
    ...
    // parts.reduce((acc, el) => {
    //   acc.push(el);
    //   const currel = this.tree.getDeepChild(acc);
    //   if (!currel) {
    ...
    if (path.extname(dest).length > 0) { // путь до файла
      this.writeFileSync(dest, body);
    } else { // путь до папки
      this.writeFileSync(`${dest}${path.sep}${current.getMeta().getName()}`, body);
    }
  }
  // END
